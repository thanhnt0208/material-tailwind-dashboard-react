...
export function Farms() {
  ...
  <div className="flex items-center justify-between mb-6">
    ...
    <Button color="indigo" className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700">
  ...
  {loading && <Typography className="text-indigo-500">Đang tải dữ liệu...</Typography>}
  ...
  {!loading && !error && (
    <CardBody className="overflow-x-auto p-0">
      <table className="w-full min-w-[1100px] table-auto">
        <thead>
          <tr className="text-left bg-indigo-50 text-indigo-600">
            <th className="w-10 px-4">
              <Checkbox ripple={false} containerProps={{ className: "p-0" }} color="indigo" />
            </th>
            {[
              "TÊN FARM",
              "MÃ FARM",
              ...
              <th key={head} className="py-3 px-4">
                <Typography
                  variant="small"
                  className="text-[11px] font-bold uppercase text-indigo-600"
                >
                  {head}
                </Typography>
              </th>
        ...
        <tbody>
          {filteredFarms.map((farm, idx) => (
            <tr
              key={farm.id}
              className={`hover:bg-indigo-50 transition ${idx !== filteredFarms.length - 1 ? "border-b" : ""}`}
            >
              <td className="py-4 px-4">
                <Checkbox ripple={false} containerProps={{ className: "p-0" }} color="indigo" />
              </td>
              <td className="py-4 px-4 font-semibold text-indigo-800">{farm.name}</td>
